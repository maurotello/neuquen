<div class="col-md-11" style="border-bottom: 2px solid #cccccc">
      <div class="row">
         <div class="form-group col-sm-5">
                 ...
                 {!! Form::text('fechaEnvioBanco', null, ['class' =>'form-control', 'readonly'] ) !!}
         </div>
         <div class="form-group col-sm-5">
                 {!! Form::label('fechaRespuestaBanco', 'Respuesta Banco') !!}
                 {!! Form::text('fechaRespuestaBanco', null, ['class' => 'form-control', 'readonly']) !!}
         </div>
      </div>
      <hr />
      ...
      <table id="table" class="table table-responsive mdl-data-table">
          ...
          @foreach($desembolsos as $x)
              <tr>
                        ...
                        <td style="width: 5%">
                                <button id="delete_desembolso" type="button" class="btn btn-info btn-xs pull-rigth delete_desembolso" onclick="borrar_desembolso({{ $x->id  }});"><i class="fa fa-trash" title="Borrar Desembolso"></i></button>
                        </td>
          ...
          <tbody>
            ...
            @foreach($sujetoCredito as $x)
                  ...
                  <td style="width: 5%">
                    ...
                    <td style="width: 5%">
                        <button id="delete_sujeto" type="button" class="btn btn-info btn-xs pull-rigth delete_sujeto" onclick="borrar_sujeto({{ $x->id  }});"><i class="fa fa-trash" title="Borrar Sujeto"></i></button>
                    </td>
                
                    <td style="width: 50%">{{ $x->sucursal->nombre }}</td>
                  @if ($x->fecha_envio_banco)
                        <td style="width: 15%">{{ \Carbon\Carbon::parse($x->fecha_envio_banco)->format('d-m-Y') }}</td>
                  @else
                        <td style="width: 15%"></th>
                  @endif
                  @if ($x->fecha_respuesta_banco)
                        <td style="width: 15%">{{ \Carbon\Carbon::parse($x->fecha_respuesta_banco)->format('d-m-Y') }}</td>
                  @else
                        <td style="width: 15%"></th>
                  @endif
                  <td style="width: 10%">{{ $x->sujeto_credito }}</td>
